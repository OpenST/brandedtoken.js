...
class Facilitator {
  ...
  async acceptStakeRequest(
    ...
    logger.info(`approveForBounty status:  ${approveForBountyReceipt.status}`);

    if (!approveForBountyReceipt.status) {
      const message = `Approval for bounty is failed with transactionHash: ${approveForBountyReceipt.transactionHash}`;
    ...
    logger.info(`acceptStakeRequest status: ${acceptStakeRequestReceipt.status}`);

    if (!acceptStakeRequestReceipt.status) {
      const message = `Accept stake request failed with transactionHash: ${acceptStakeRequestReceipt.transactionHash}`;
      logger.error(message);
      const err = new Error(
        message,
      );
