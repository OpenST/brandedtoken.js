...
class StakeHelper {
  ...
  acceptStakeRequest(
    ...
    //1. Create TypedData
    const stakeRequestTypedData = oThis._getStakeRequestTypedData(stakeAmountInWei, btStakeRequestNonce);

    //2. Generate EIP712 Signature.
    const workerAccountInstance = web3.eth.accounts.wallet[workerAddress];
    const stakeRequestSignedHash = workerAccountInstance.signEIP712TypedData(stakeRequestTypedData);

    //3. Create Tx Object.
    const txObject = oThis._acceptStakeRequestRawTx(
      stakeRequestHash,
      stakeRequestSignedHash,
      facilitator,
      hashLock,
      web3,
      txOptions
   ...
   * @param stakeRequestSignedHash EIP712 Signature of KYC worker address generated by signing StakeRequest typed-data.
   * @param facilitator Facilitator address.
   * @param hashLock HashLock of facilitator.
   * @param [originWeb3] Origin chain web3 object.
   ...
   */
  _acceptStakeRequestRawTx(stakeRequestHash, stakeRequestSignedHash, facilitator, hashLock, originWeb3, txOptions) {
    const oThis = this;
    ...
    const txObject = Contract.methods.acceptStakeRequest(
      stakeRequestHash,
      stakeRequestSignedHash.r,
      stakeRequestSignedHash.s,
      stakeRequestSignedHash.v,
      hashLock
    );
   ...
   * @private
   */
  _getStakeRequestTypedData(stakeAmountInWei, btStakeRequestNonce, gatewayComposer, brandedToken) {
    const oThis = this;
    gatewayComposer = gatewayComposer || oThis.gatewayComposer;
    brandedToken = brandedToken || oThis.brandedToken;
