...
class StakeHelper {
  ...
  approveForValueToken(
    ...
    if (valueTokenAbi.length === 0) {
      const message = 'Value token abi is not provided';
   ...
   * @returns {EIP712Signature} Signature generated after EIP712 signing.
   * @private
   */
  getStakeRequestTypedData(stakeAmountInWei, btStakeRequestNonce, gatewayComposer, brandedToken) {
    ...
    const gatewayComposerAddr = gatewayComposer || this.gatewayComposer;
    const brandedTokenAddr = brandedToken || this.brandedToken;

    return Utils.getStakeRequestTypedData(
      stakeAmountInWei,
      btStakeRequestNonce,
      gatewayComposerAddr,
      brandedTokenAddr,
