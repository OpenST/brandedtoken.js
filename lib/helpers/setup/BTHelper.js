...
class BTHelper {
  ...
  setup(config, txOptions, originWeb3) {
    ...
    let valueToken; let symbol; let name; let decimals; let conversionRate; let conversionRateDecimals; let
      organization;
  ...
  _deployRawTx(
    ...
    const args = [valueToken, symbol, name, decimals, conversionRate, conversionRateDecimals, organization];
  ...
  setGateway(gateway, organizationWorker, txOptions, contractAddress, originWeb3) {
    ...
    const gatewayContract = Contracts.getEIP20Gateway(originWeb3, gateway);

    return gatewayContract.methods
      .stakeVault()
      .call()
      .then(stakeVault => oThis.liftRestriction([gateway, stakeVault], organizationWorker, txOptions, contractAddress, originWeb3));
  }
  ...
  liftRestriction(addresses, organizationWorker, txOptions, contractAddress, originWeb3) {
    ...
    const tx = oThis._liftRestrictionRawTx(addresses, organizationWorker, txOptions, contractAddress, originWeb3);
