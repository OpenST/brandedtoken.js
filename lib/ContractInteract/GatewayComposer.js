...
class GatewayComposer {
  ...
  constructor(web3, address) {
    ...
    if (!this.contract) {
      throw new TypeError(
        `Could not load gateway composer contract for: ${this.address}`,
      );
    }
   ...
   *
   * @param stakeRequestHash Hash of stake request information calculated per
   *                         EIP 712.
   ...
   * @param s s of signature received from worker.
   * @param v v of signature received from worker.
   * @param hashLock 32 bytes value provided by the facilitator.
   *
   ...
   * @param penalty amount of penalty in wei charged to staker for revert.
